...
for ($i=0; $i < count($words); $i++){
	...
	debugPrint("<hr><p> onewordsresults: " . var_dump($oneWordsResults));
...
/// look for words
	...
	foreach($allWordsAllResults as $key=>&$val){ // words complete json
		...
		foreach($val["items"] as $key=>&$item){ // one word's items
			$b++;
			$c= -1;
			foreach($item as $key=>&$modstring){ // the returns for one word
				$c++;
				debugPrint("count: " . count($modstring));
				//$resultstring = $modstring.join(", ");
				for ($y=0; $y < count($modstring); $y++){
					debugPrint($y . "::" . $modstring[$y]);
				}
				//debugPrint("mods: " . var_dump($modstring));
				$resultstring = $modstring;
				debugPrint("resulstring=" . print($resultstring));
